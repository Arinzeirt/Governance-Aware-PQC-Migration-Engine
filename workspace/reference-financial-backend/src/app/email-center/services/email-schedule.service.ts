...
export class EmailJobWorker implements OnModuleInit, OnModuleDestroy {
  ...
  private async sendEmail(data: string | EmailJob): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < payload.users.length; i += batchSize) {
        const batch = payload.users.slice(i, i + batchSize);

        const results = await Promise.allSettled(
          batch.map(async ({ email, fullName }) => {
            let attempt = 0;
            const compiledInner = handlebars.compile(payload.message);
            const personalizedContent = compiledInner({
            ...
            while (attempt < maxRetries) {
              try {
                await this.mailService.sendMail(
                  ...
                  payload.template,
                  payload.subject,
                  { body: personalizedContent, username: fullName },
                  true,
                );
                return; // success
              } catch (err) {
                attempt++;
                console.error(
                  `Failed to send to ${email}, attempt ${attempt}:`,
                  err,
                );
