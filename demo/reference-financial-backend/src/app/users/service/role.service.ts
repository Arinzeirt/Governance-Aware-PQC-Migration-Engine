...
import { ChangeUserRoleDto } from '../dto/toggle-role.dto';
...
export class RoleService {
  ...
  async changeUserRole(payload: ChangeUserRoleDto): Promise<User> {
    ...
    const newRole = await this.role.findOne({
      where: { name: payload.roleName },
    });

    if (!newRole) throw new Error('Role not found');

    user.roleLevel = newRole.roleLevel;
    user.role = newRole.name;
    user.roles = newRole;
    user.type = newRole.type;
