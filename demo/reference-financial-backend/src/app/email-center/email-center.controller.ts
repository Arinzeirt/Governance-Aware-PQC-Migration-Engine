...
import { CreateEmailCenter, EmailSubscription } from './dto';
import {
  FindManyEmail,
  FindOneEmail,
...
export class EmailCenterController {
  ...
  @Public()
  @ApiOperation({ summary: 'Subscribe to email list' })
  @ApiBody({ type: EmailSubscription })
  emailSubscribing(@Body() payload: EmailSubscription) {
    return this.emailCenterService.emailSubscribing(payload);
  }
}
