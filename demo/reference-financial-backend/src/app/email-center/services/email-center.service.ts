...
export class EmailCenterService {
      ...
      this.emailJobWorker.scheduleDelayedEmail(
        {
          ...
          message: update.message ?? findEmail.message,
        },
        delay,
      );
  ...
  async emailSubscribing(payload: emailSubscription): Promise<WaitList> {
    try {
      const email = await this.wistList.findOne({
        where: { email: payload.email },
      });
      if (email) {
        throw new BadRequestException('Email already subscribed');
      }
      const waitList = this.wistList.create(payload);
      return this.wistList.save(waitList);
    } catch (error) {
