...
import { ConvertService, USDCService, WalletsService } from './service';
import { SeerBitService } from './service/seerbit.service';
...
export class WalletsController {
  ...
  @Post('convert-asset')
  async convertAsset(@Body() payload: Convert, @SessionUser() user: AuthUser) {
    return this.convertService.convertAsset(payload, user);
  }
  ...
  @Get('create-virtual-account')
  @ApiOperation({ summary: 'Create Virtual Naira Account' })
  async createAccount(@SessionUser() user: AuthUser) {
    return this.seerbitService.createVirtualAccount(user);
  }

  @Get('nigeria-banks-list')
  @ApiOperation({ summary: 'Get the list of nigeria banks' })
  async NigeriaBanksList() {
    return this.seerbitService.bankList();
  }

  @Post('name-enquiry')
  @ApiBody({ type: NameEnquiry })
  @ApiOperation({ summary: 'Get the account name of the beneficiary' })
  async NameEnquiry(@Body() payload: NameEnquiry) {
    return this.seerbitService.nameEnquiry(payload);
  }

  @Post('nigeria-transfer')
  @ApiBody({ type: TransferFunds })
  @ApiOperation({ summary: 'Transfer to Nigeria Banks' })
  NairaTransfer(@Body() payload: TransferFunds) {
    console.log(payload);
    return { message: 'Transaction cant be done now!' };
    // return this.seerbitService.TransferFund()
  }

  // @Public()
  // @Get('generate-secret')
  // async generateSecret() {
  //   return this.usdcService.generateEnSecretCiphertext();
  // }
}
