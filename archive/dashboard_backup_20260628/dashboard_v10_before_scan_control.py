...
if df.empty:
    st.warning("Inventory is empty. Run a scan first.")
    st.stop()

classified_df = df[df["finding_type"] == "classification"].copy()

if classified_df.empty:
    st.warning("No classified findings found.")
    st.stop()
...
with st.sidebar.expander("Priority", expanded=True):
    ...
    selected_priorities = st.multiselect("Select Priority", priorities, default=priorities)
...
with st.sidebar.expander("Asset Type", expanded=True):
    ...
    selected_asset_types = st.multiselect("Select Asset Type", asset_types, default=asset_types)
...
with st.sidebar.expander("Owner", expanded=True):
    ...
    selected_owners = st.multiselect("Select Owner", owners, default=owners)
...
with st.sidebar.expander("Migration Status", expanded=True):
    ...
    selected_statuses = st.multiselect("Select Migration Status", statuses, default=statuses)
...
with main_left:
    with st.container(border=True):
        ...
        with input_col1:
            ...
            protection_horizon = st.selectbox(
                "Protection Horizon",
        ...
        with input_col2:
            governance_ready = st.checkbox(
                "Governance Approved",
                value=(under_review == 0)
            )

            compliance_ready = st.checkbox(
                "Compliance Ready",
                value=False
            )

            audit_ready = st.checkbox(
